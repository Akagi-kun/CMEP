...
namespace Engine::Rendering::Factories
{
	VkImageView VulkanImageFactory::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
	{
		...
		if (vkCreateImageView(this->vulkanRenderingEngine->GetLogicalDevice(), &view_info, nullptr, &image_view) !=
			VK_SUCCESS)
		{
	...
	{
		VkCommandBuffer command_buffer = this->vulkanRenderingEngine->BeginVulkanSingleTimeCommandsCommandBuffer();
		...
		this->vulkanRenderingEngine->EndVulkanSingleTimeCommandsCommandBuffer(command_buffer);
	}
} // namespace Engine::Rendering::Factories
