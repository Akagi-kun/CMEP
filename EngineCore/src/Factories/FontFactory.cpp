...
namespace Engine::Factories
{
	...
	std::shared_ptr<Rendering::Font> FontFactory::InitBMFont(std::string fontPath)
	{
		...
		if(!fontFile.is_open())
		{
			this->logger->SimpleLog(Logging::LogLevel::Error, LOGPFX_CURRENT "FontFile %s unexpectedly not open", fontPath.c_str());
			return nullptr;
		}
		...
		this->logger->SimpleLog(Logging::LogLevel::Debug2, LOGPFX_CURRENT "File %s loaded successfully", fontPath.c_str());
